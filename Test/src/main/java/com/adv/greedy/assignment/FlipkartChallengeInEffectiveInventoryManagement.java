...
public class FlipkartChallengeInEffectiveInventoryManagement {
    ...
    public static int solve(ArrayList<Integer> A, ArrayList<Integer> B) {
        ...
        for(int i =0 ;i < A.size();i++) {
            profitList.add(new Profit(A.get(i),B.get(i)));
        }
        profitList = profitList.stream().sorted(Comparator.comparingInt(value -> value.duration)).collect(java.util.stream.Collectors.toList());
        PriorityQueue<Integer> minheap = new PriorityQueue<>();

        int time =0;
        for(int i=0 ;i< A.size();i++) {
            if(profitList.get(i).duration > time) {
                minheap.add(profitList.get(i).profit);
                time++;
            } else {
                if(profitList.get(i).profit > minheap.peek()) {
                    minheap.poll();
                    minheap.add(profitList.get(i).profit);
                }
            }
        ...
        while (!minheap.isEmpty()) {
            totalSum = (totalSum + minheap.poll()) % 1000000007;
        }
        return totalSum;
    }
}
