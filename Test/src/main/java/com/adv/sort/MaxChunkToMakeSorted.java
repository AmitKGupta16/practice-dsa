...
/*Problem Description
        ...
        and individually sort each chunk. After concatenating them in order of splitting,
...
public class MaxChunkToMakeSorted {
    ...
    public static int solve(ArrayList<Integer> A) {
        int cnt=0,max = Integer.MIN_VALUE,index=0;
        for(Integer x : A) {
            max = Math.max(max,x);
            if(max==index) {
                cnt++;
    ...
    public static void main(String[] args) {
        System.out.println(solve(new ArrayList<>(List.of(2, 0, 1, 3))));
    }
